...
float lookup(vec2 p, float dx, float dy) {
    vec2 uv = (p.xy + vec2(dx * d, dy * d)) / u_TexSize.xy;
    if (u_Mirror) uv.x = 1.0-uv.x;
    uv.y = 1.0-uv.y;
    vec4 c = texture(s_Texture, uv.xy);
...
void main() {
    ...
    float coordX = fragCoord.x / u_TexSize.x;
    float coordY = fragCoord.y / u_TexSize.x;
