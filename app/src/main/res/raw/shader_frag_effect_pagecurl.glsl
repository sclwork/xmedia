...
vec4 seeThrough(float yc, vec2 p, mat3 rotation, mat3 rrotation) {
    ...
    if (yc <= 0.0 && (point.x < 0.0 || point.y < 0.0 || point.x > 1.0 || point.y > 1.0))
        return bgColor;
    if (yc > 0.0)
        return texture(s_Texture, p);
    vec4 color = texture(s_Texture, point.xy);
...
void main() {
    vec2 uv = v_texCoord;
    bgColor = texture(s_Texture, uv).rgga;
    curlExtent = sin(u_Time/16.0*0.3)*0.5+0.5;
        ...
        if ((hitAngleMod > PI && amount < 0.5) || (hitAngleMod > PI/2.0 && amount < 0.0)) {
            ...
            if (point.x < 0.0 || point.y < 0.0 || point.x > 1.0 || point.y > 1.0) {
                ...
                if (yc < 0.0) {
                    ...
                    shado *= 0.5;
                    otherColor = vec4(0.6, 0.6, 0.6, shado);
                } else {
                    otherColor = texture(s_Texture, uv);
                }
